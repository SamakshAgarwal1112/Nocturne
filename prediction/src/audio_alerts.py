...
class AudioAlerts:
    ...
    def __init__(self, normal_message="Hey, are you awake?", 
        ...
        Args:
            ...
            piper_binary (str): Path to Piper binary
            piper_model (str): Path to Piper voice model
            vosk_model (str): Path to Vosk model
            phi3_model (str): Path to Phi-3 Mini model
        """
        ...
        self.llm = Llama(model_path=phi3_model, n_ctx=512, n_threads=4)
        ...
        self.phi3_channel = pygame.mixer.Channel(3)
    ...
    def _send_to_phi3(self, user_speech, drowsiness_level):
        """
        Send user speech and drowsiness level to Phi-3 Mini model
        ...
        Returns:
            dict: Parsed JSON response with 'convinced' (bool) and 'message' (str)
        """
        prompt = f"""
        ...
        2. "message": if not convinced, include a brief, human-sounding message to engage the driver
        Keep the message short, clear, and interesting to prompt a response. Create familiarity with the driver, occasionally adding a touch of playful or engaging tone.
        
        Example response if convinced:
        {{"convinced": true}}
        
        Example response if not convinced:
        {{"convinced": false, "message": "Hey bud, you sound a bit out of it! What's your favorite song to stay pumped?"}}
        """
        
        try:
            output = self.llm(prompt, max_tokens=100, temperature=0.7)
            result = json.loads(output["choices"][0]["text"])
            return result
        except Exception as e:
            print(f"Error processing with Phi-3 Mini: {e}")
            # Fallback: Keyword-based alertness check
            alert_keywords = ["fine", "okay", "awake", "alert", "good", "coffee"]
            convinced = any(keyword in user_speech.lower() for keyword in alert_keywords)
            message = "Alright, let's keep those eyes on the road!" if convinced else "You sound tired, bud. Maybe sing a tune to stay sharp?"
            return {"convinced": convinced, "message": message}
    ...
    def _process_voice_with_phi3(self, audio_data):
        """Process voice input with Phi-3 Mini with echo cancellation"""
        try:
            ...
            # Send to Phi-3 Mini
            phi3_response = self._send_to_phi3(user_speech, self.current_drowsiness_level)
            self.system_alert_active = True

            # Process response
            if phi3_response.get("convinced", False):
                ...
                if confirm_sound:
                    self.phi3_channel.play(confirm_sound)
                return True
            else:
                # Play custom message from Phi-3
                message = phi3_response.get("message", "I'm not convinced you're fully alert. Please continue focusing.")
                print(f"Phi-3 response: {message}")
                
                response_sound = self._generate_temp_audio(message)
                if response_sound:
                    self.stop_all_alerts()
                    self.phi3_channel.play(response_sound)
                return False
                
        except Exception as e:
            print(f"Error processing voice with Phi-3: {e}")
            return False
    ...
    def _listen_for_response(self):
        ...
        while not self.stop_voice_detection:
            try:
                ...
                is_alert = self._process_voice_with_phi3(audio_data.flatten())
                if is_alert:
                    self.system_alert_active = False
                    break
